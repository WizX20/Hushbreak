...
& 'C:\Program Files\VideoLAN\VLC\vlc.exe' -I dummy --extraintf luaintf --lua-intf hushbreak `
    ...
    --no-one-instance --audio-filter gain --gain-value 0 --verbose 2 --file-logging --logfile hushbreak-test.log KINK.pls
```

Within a second the log shows `ad break: volume 256 -> 102 (BEEQUIP FINAL)`, then `next spot: KV WK38 ALWAYS`, `next spot: INTERPOLIS GRIPOPCYBER`, and 111 s after the last spot `ad break over: volume back to 256` — the fixture carries the song after the block (Supersonic, 114 s after the last spot), and the fade-up starts `fade_up` seconds before the listener reaches it. The gain filter at 0 keeps it silent, `--no-one-instance` keeps it away from the VLC you listen with. Do not reach for `--aout amem`, `adummy` or `afile` instead: `amem` needs the libvlc callbacks and falls back to the real output (audible), the other two have no working volume (`vlc.volume.get()` stays 0), so `mmdevice` is the only output on which the script's fades can be observed. Stop it by its own PID; never kill every `vlc` process, the listener's VLC is one of them.

Two things to know about that instance:

- **It shares the volume Windows remembers for `vlc.exe`.** `mmdevice` writes every `vlc.volume.set` into the Windows audio session, and a fresh VLC starts at the level the previous one left, so a test instance stopped while ducked hands that level to the listener's next VLC start. Read the level before the run (a throwaway interface script that logs `vlc.volume.get()` in an instance started with `--no-audio`), put it back the same way afterwards, and prefer quitting over killing.
...
- **VLC takes 1–5 s to connect, with the playback position at 0.** Measured on the same machine on consecutive starts: `input` exists, `time` stays 0 until the stream's first bytes are decoded ("Stream buffering done" in the log). Counting that as drift made every fresh start's fades late by the connect time, on top of the base delay that already covers the burst and `network-caching`. The drift reference is therefore taken at the first advancing position (`Drift.pending`), at startup and after every restart.
