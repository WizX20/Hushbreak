...
- **The insert trigger is not the end of the commercials.** After the last spot the feed carries an `ad` entry with `ad_type=insert` (title "COMMERCIAL INSERT TRIGGER …") whose `cue_time_start` equals the last spot's end. It marks the end of Triton's own spots only: in the fixture's block (13:00 CEST) it is followed by an untitled 8 s entry, an untitled 71 s entry that the listener heard as commercials, four untitled jingles (35 s), and the first titled song **114 s after the last spot**. Un-ducking on the trigger brought the volume up ~1 minute early (v0.1.1). `block_ended` therefore waits for a titled song after the last spot and otherwise for `grace` (180 s, measured 114 s plus room for a longer station block and the feed's lag variance).
