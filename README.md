...
| `grace` | `180` | Seconds to stay ducked after the last known spot when the feed has not shown a song yet. Commercials go on after Triton's last spot (measured on KINK: 114 s until the song), breaks can hold promo segments between two runs of spots, and the feed server stalls for seconds at a time; a long grace keeps the volume from coming up in the middle of the commercials. |
...
- Every entry in the feed (song, jingle, spot, "commercial insert" trigger) has a start and a duration, so Hushbreak knows when the current entry ends and polls just after that moment instead of on a fixed interval — once per song, once per spot. Ad blocks are made of contiguous spots. The "commercial insert" trigger that follows the last spot marks the end of Triton's own spots, not of the commercials: on KINK about 80 s of station commercials and a few jingles follow it before the music resumes.
- The volume goes back up only when the end of the break is certain: the feed shows a titled song after the last spot (jingles, news and promos carry no title) and you have reached it — the fade-up ends exactly on the song. Without a song, Hushbreak stays ducked for `grace` seconds — a song that stays soft a little longer after a missed song entry beats the volume coming up in the middle of the commercials.
